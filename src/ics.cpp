...
int ics_read(ICSData * r, int n)
{
	assert(r);
	int i;
	if ((i = ftdi_read_data(&r->ftdic, r->swap, n)) < 0)
		ics_ftdi_error(r);
	return i;
}
/*-----------------------------------------------------------------------------
...
/*-----------------------------------------------------------------------------
 * Get temperature parameter of the servo
 * id: the id of the servo 0-31
 * Returns: Value of temperature (1 - 127), or < 0 if error
 */
int ics_get_temperature(ICSData * r, UINT id)
{
	assert(r);
	int i;
	...
	r->swap[0] = id | ICS_CMD_GET; // id and command
	r->swap[1] = ICS_SC_TMP; // subcommand
...
/*-----------------------------------------------------------------------------
 * Get position parameter of the servo
 * id: the id of the servo 0-31
 * Returns: Value of angle (3500 - 11500), or < 0 if error
 */
int ics_get_position(ICSData * r, UINT id)
{
	assert(r);
	int i;
	...
	r->swap[0] = id | ICS_CMD_GET; // id and command
	r->swap[1] = ICS_SC_TCH; // subcommand
	
	// synchronize
	if ((i = ics_trx_timeout(r, 2, 6, ICS_GET_TIMEOUT)) < 0)
		return i;
